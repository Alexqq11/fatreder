class FatBootSector:
    ...
    def get_fat_offsets_list(self):
        fats_offsets = []
        for fat_number in (1, self.bpb_number_fats + 1):
    ...
    def _read_fat_boot_sector(self):
        ...
        self.bpb_fat_size_16 = self.reader.get_data_local(22, 2,
                                                          True)  # 22 2 amount fat sectors for one fat12/16 table
        self.bpb_sectors_per_track = self.reader.get_data_local(24, 2,
                                                                True)  # 24 2 for interrupt 13 and accses to disks
        self.bpb_number_heads = self.reader.get_data_local(26, 2, True)  # 26 2 ammount of disk heads
        ...
        # there was can been fat12/16 fields but we starting write fat 32 fields
        self.bpb_fat_size_32 = self.reader.get_data_local(36, 4, True)  # 36 4 amoun of sectors one fat
        ...
        self.bpb_file_system_information = self.reader.get_data_local(48, 2,
