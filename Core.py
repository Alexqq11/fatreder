...
class FatBootSector:
    ...
    def _read_fat_boot_sector(self):
        ...
        self.bpb_sectors_per_cluster = self.reader.get_data(12,1,True)  # 13 1
        ...
        self.bpb_total_sectors_16 = self.reader.get_data(19,2,True) # 19 2 old sixteen bits field in fat 32 must be zero
        self.bpb_media = self.reader.get_data(21,1)  # 21 1 stand
        self.bpb_fat_size_16 = self.reader.get_data(22,2,True)  # 22 2 amount fat sectors for one fat12/16 table in fat32 zero watch to fat 32
        self.bpb_sectors_per_track = self.reader.get_data(24,2,True) # 24 2 for interrupt 13 and accses to disks with geometry #old tech
        ...
        self.bpb_file_system_information = self.reader.get_data(48,2,True)  # 48 2 кол-во свободных кластеров на диске, чтобы утилитам легче жить
...
class ImageReader:
    ...
    def get_data(self, offset, size, convert_integer=False):
        self.image.seek(offset)
        buffer = self.image.read(size)
        if(convert_integer):
            buffer = struct.unpack(self._get_parse_mod(size),buffer)[0]
        return buffer
